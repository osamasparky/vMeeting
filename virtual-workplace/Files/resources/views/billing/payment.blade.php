...
<div style="position: absolute; top: 20px; inset-inline-end: 24px; z-index: 10; display: flex; gap: 10px; align-items: center;">
    @if(app()->getLocale() === 'ar')
        <a href="{{ route('lang.switch', 'en') }}" class="lang-switch-btn" style="background: #ffffff; border: 1px solid var(--ula-border-subtle); color: var(--ula-palm-900); padding: 7px 14px; border-radius: 8px; font-size: 13px; text-decoration: none; font-weight: 800; box-shadow: var(--ula-shadow-xs);">🌐 English</a>
    @else
        <a href="{{ route('lang.switch', 'ar') }}" class="lang-switch-btn" style="background: #ffffff; border: 1px solid var(--ula-border-subtle); color: var(--ula-palm-900); padding: 7px 14px; border-radius: 8px; font-size: 13px; text-decoration: none; font-weight: 800; box-shadow: var(--ula-shadow-xs);">🌐 العربية</a>
    @endif
...
<div class="auth-wrapper" style="min-height: 100vh; padding: 40px 20px; max-width: 1200px; margin: 0 auto; display: flex; flex-direction: column; justify-content: center;">
    ...
    <div style="display: flex; justify-content: center; align-items: center; gap: 16px; margin-bottom: 28px; flex-wrap: wrap;">
        <div style="display: flex; align-items: center; gap: 8px; opacity: 0.6;">
            <span style="width: 28px; height: 28px; border-radius: 50%; background: var(--ula-palm-900); color: white; display: flex; align-items: center; justify-content: center; font-size: 12px; font-weight: 800;">✓</span>
            <span style="font-size: 13px; font-weight: 800; color: var(--ula-text-secondary);">1. {{ __('Choose Plan') }}</span>
        </div>
        <span style="color: var(--ula-border-subtle); font-weight: 800;">➔</span>
        <div style="display: flex; align-items: center; gap: 8px;">
            <span style="width: 28px; height: 28px; border-radius: 50%; background: var(--ula-gradient-accent); color: white; display: flex; align-items: center; justify-content: center; font-size: 12px; font-weight: 800; box-shadow: 0 4px 10px rgba(36,92,58,0.3);">2</span>
    ...
    <div style="display: grid; grid-template-columns: 1fr 1.2fr; gap: 24px; align-items: start;">
        ...
        <div style="display: flex; flex-direction: column; gap: 20px;">
            ...
            <div style="background: var(--ula-surface-card); border: 2px solid var(--ula-palm-900); border-radius: var(--ula-radius-xl); padding: 24px; box-shadow: var(--ula-shadow-xs); position: relative;">
                <div style="position: absolute; top: -12px; inset-inline-end: 20px; background: var(--ula-palm-900); color: white; padding: 4px 14px; border-radius: 9999px; font-size: 11px; font-weight: 900; text-transform: uppercase;">
                ...
                <h2 style="font-size: 24px; font-weight: 900; color: var(--ula-text-primary); margin-bottom: 8px;">💎 {{ $plan->name }} {{ __('Plan') }}</h2>
                ...
                <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 10px; margin-bottom: 16px;">
                    <div style="background: var(--ula-surface-page-alt); padding: 10px 14px; border-radius: 10px; border: 1px solid var(--ula-border-subtle);">
                        <div style="font-size: 10px; font-weight: 800; color: var(--ula-text-muted); text-transform: uppercase;">👥 {{ __('Seats Capacity') }}</div>
                    ...
                    <div style="background: var(--ula-surface-page-alt); padding: 10px 14px; border-radius: 10px; border: 1px solid var(--ula-border-subtle);">
                        <div style="font-size: 10px; font-weight: 800; color: var(--ula-text-muted); text-transform: uppercase;">🏢 {{ __('Meeting Rooms') }}</div>
                    ...
                    <div style="background: var(--ula-surface-page-alt); padding: 10px 14px; border-radius: 10px; border: 1px solid var(--ula-border-subtle);">
                        <div style="font-size: 10px; font-weight: 800; color: var(--ula-text-muted); text-transform: uppercase;">💾 {{ __('Cloud Storage') }}</div>
                    ...
                    <div style="background: var(--ula-surface-page-alt); padding: 10px 14px; border-radius: 10px; border: 1px solid var(--ula-border-subtle);">
                        <div style="font-size: 10px; font-weight: 800; color: var(--ula-text-muted); text-transform: uppercase;">⚡ {{ __('Activation') }}</div>
                        <div style="font-size: 14px; font-weight: 900; color: var(--ula-palm-900);">{{ __('Instant SuperAdmin Review') }}</div>
                    </div>
                </div>

                <div style="font-size: 12px; font-weight: 800; color: var(--ula-text-secondary); margin-bottom: 8px;">✨ {{ __('Included Features') }}:</div>
                ...
                <div style="display: flex; flex-direction: column; gap: 14px;">
                    ...
                    <div style="background: var(--ula-surface-page-alt); border: 1px solid var(--ula-border-subtle); border-radius: 12px; padding: 16px;">
                        <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 8px;">
                            <strong style="font-size: 14px; color: var(--ula-text-primary);">🏛️ {{ $bank['bank_name'] }}</strong>
                        ...
                        <div style="display: flex; flex-direction: column; gap: 8px; font-size: 12px;">
                            @if(!empty($paymentSettings['instapay_handle']))
                                <div style="display: flex; justify-content: space-between; align-items: center; background: var(--ula-surface-page-alt); padding: 8px 12px; border-radius: 8px; border: 1px solid var(--ula-border-subtle);">
                                    <span>⚡ <strong>Instapay IPA:</strong> <code style="color: var(--ula-palm-900); font-weight: 800;">{{ $paymentSettings['instapay_handle'] }}</code></span>
                                    <button type="button" onclick="copyToClipboard('{{ $paymentSettings['instapay_handle'] }}', this)" style="background: none; border: 1px solid var(--ula-border-subtle); padding: 2px 6px; border-radius: 4px; font-size: 10px; cursor: pointer;">📋 {{ __('Copy') }}</button>
                                </div>
                            @endif
                            @if(!empty($paymentSettings['stc_pay_phone']))
                                <div style="display: flex; justify-content: space-between; align-items: center; background: var(--ula-surface-page-alt); padding: 8px 12px; border-radius: 8px; border: 1px solid var(--ula-border-subtle);">
                                    <span>💚 <strong>STC Pay:</strong> <code style="color: var(--ula-palm-900); font-weight: 800;">{{ $paymentSettings['stc_pay_phone'] }}</code></span>
                                    <button type="button" onclick="copyToClipboard('{{ $paymentSettings['stc_pay_phone'] }}', this)" style="background: none; border: 1px solid var(--ula-border-subtle); padding: 2px 6px; border-radius: 4px; font-size: 10px; cursor: pointer;">📋 {{ __('Copy') }}</button>
                                </div>
                            @endif
                            @if(!empty($paymentSettings['vodafone_cash_phone']))
                                <div style="display: flex; justify-content: space-between; align-items: center; background: var(--ula-surface-page-alt); padding: 8px 12px; border-radius: 8px; border: 1px solid var(--ula-border-subtle);">
                                    <span>🔴 <strong>Vodafone Cash:</strong> <code style="color: var(--ula-palm-900); font-weight: 800;">{{ $paymentSettings['vodafone_cash_phone'] }}</code></span>
                                    <button type="button" onclick="copyToClipboard('{{ $paymentSettings['vodafone_cash_phone'] }}', this)" style="background: none; border: 1px solid var(--ula-border-subtle); padding: 2px 6px; border-radius: 4px; font-size: 10px; cursor: pointer;">📋 {{ __('Copy') }}</button>
                                </div>
                            @endif
                        </div>
                    </div>
            ...
            <form method="POST" action="{{ route('subscription.payment.submit', $plan->id) }}" enctype="multipart/form-data" id="bankPaymentForm">
                ...
                <div style="background: var(--ula-surface-page-alt); border: 1px solid var(--ula-border-subtle); border-radius: 12px; padding: 14px; margin-bottom: 20px; display: flex; justify-content: space-between; align-items: center;">
                    <div>
                        ...
                        <div style="font-size: 14px; font-weight: 900; color: var(--ula-text-primary);">🏛️ {{ $organization->name }}</div>
                    </div>
                    <div style="text-align: end;">
                        <div style="font-size: 10px; font-weight: 800; color: var(--ula-text-muted); text-transform: uppercase;">{{ __('Target Plan') }}</div>
                        <div style="font-size: 14px; font-weight: 900; color: var(--ula-palm-900);">💎 {{ $plan->name }}</div>
                    </div>
                </div>
