    ...
    <div style="display: flex; flex-direction: column; gap: 18px;">
        @foreach($page->sections as $index => $sec)
            <div class="panel-card" style="border-radius: var(--ula-radius-xl); padding: 24px; border: 1px solid var(--ula-border-subtle); background: var(--ula-surface-card);">
                ...
                <div style="display: flex; justify-content: space-between; align-items: center; flex-wrap: wrap; gap: 12px; margin-bottom: 18px; padding-bottom: 14px; border-bottom: 1px solid var(--ula-border-subtle);">
                    <div style="display: flex; align-items: center; gap: 12px;">
                        <span style="font-size: 12px; font-weight: 900; background: var(--ula-surface-page-alt); color: var(--ula-palm-900); padding: 4px 10px; border-radius: 8px;">
                            #{{ $sec->display_order }}
                        </span>
                        <div>
                            <strong style="font-size: 16px; font-weight: 900; color: var(--ula-text-primary);">
                                {{ $sec->title_en ?: ucfirst(str_replace('_', ' ', $sec->section_type)) }}
                            </strong>
                            <span style="font-size: 11px; color: var(--ula-text-muted); display: block; font-family: monospace;">
                ...
                <form method="POST" action="{{ route('superadmin.cms.sections.update', $sec) }}">
                    ...
                    <div style="display: flex; justify-content: space-between; align-items: center;">
                        <label style="display: flex; align-items: center; gap: 8px; font-size: 12px; font-weight: 800; color: var(--ula-text-primary); cursor: pointer;">
                            <input type="checkbox" name="is_active" value="1" {{ $sec->is_active ? 'checked' : '' }} style="accent-color: var(--ula-palm-900);">
