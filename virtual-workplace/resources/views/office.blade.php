...
<body>
    ...
    <!-- Instant Guest Invite Modal -->
    <div id="invite-modal" class="modal-overlay">
        <div class="modal-box">
            <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 16px;">
                <h3 style="font-size: 16px; font-weight: 700;">🔗 Instant Guest Invitation</h3>
    ...
    <script>
        ...
        const remoteAvatars = new Map(); // userId -> { id, name, x, y, targetX, targetY, color, status }
        ...
        // ── User Input & Movement (WASD, Arrows, Click-to-move) ──
        const keys = {};
        ...
        function update() {
            ...
            if (dx !== 0 || dy !== 0) {
                ...
                if (dist > 3) {
                    localAvatar.x += (diffX / dist) * localAvatar.speed;
                    localAvatar.y += (diffY / dist) * localAvatar.speed;
                    moved = true;
                }
            }

            localAvatar.x = Math.max(localAvatar.radius, Math.min(1300, localAvatar.x));
            localAvatar.y = Math.max(localAvatar.radius, Math.min(950, localAvatar.y));
        ...
        function draw() {
            try {
                ...
                rooms.forEach(r => {
                    ...
                    ctx.fillRect(rx + 8, ry + 8, 140, 26);
                    ctx.strokeStyle = 'rgba(255, 255, 255, 0.2)';
                    ctx.lineWidth = 1;
                    ctx.strokeRect(rx + 8, ry + 8, 140, 26);
        ...
        // ── Toolbar & Control Dock Listeners ──
        ...
        async function toggleCamera() {
            ...
            if (!isCamActive) {
                try {
                    const stream = await navigator.mediaDevices.getUserMedia({ video: { width: 320, height: 240 }, audio: isMicActive });
        ...
        async function toggleMicrophone() {
            ...
            if (!isMicActive) {
                try {
                    const stream = await navigator.mediaDevices.getUserMedia({ audio: true, video: isCamActive });
                    localMediaStream = stream;
                    isMicActive = true;
                    btn.classList.add('active');
                } catch(e) {
                    alert('Microphone access: ' + e.message);
                }
            } else {
                isMicActive = false;
                btn.classList.remove('active');
                if (localMediaStream) {
                    localMediaStream.getAudioTracks().forEach(t => t.stop());
                }
            }
        }
        ...
        // ── Invite Modal & Guest Link Generation ──
