    ...
    <div class="page-header" style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 24px; flex-wrap: wrap; gap: 14px;">
        <div>
            <h1 class="page-title" style="font-size: 22px; font-weight: 800; color: var(--nx-text-primary); margin-bottom: 4px; display: flex; align-items: center; gap: 8px;">
                <span class="material-symbols-rounded" style="font-size: 24px; color: var(--nx-accent);">group</span>
                <span>{{ __('Team Members & Roles') }}</span>
            </h1>
            <p class="page-subtitle" style="font-size: 13px; color: var(--nx-text-secondary);">{{ __('Manage organization membership, departments, teams, and security roles.') }}</p>
        </div>
    ...
    <div class="card" style="border-radius: var(--nx-radius-lg); overflow: hidden; padding: 0;">
        <div style="padding: 20px 24px; border-bottom: 1px solid var(--nx-border-subtle); display: flex; justify-content: space-between; align-items: center; background: var(--nx-bg-surface);">
            <h3 style="font-size: 16px; font-weight: 800; color: var(--nx-text-primary); display: flex; align-items: center; gap: 8px;">
                <span class="material-symbols-rounded" style="font-size: 18px; color: var(--nx-accent);">badge</span>
                        ...
                        <tr>
                            <td>
                                <div onclick="openMemberProfileModal('{{ $m->id }}')" style="display: flex; align-items: center; gap: 10px; cursor: pointer; transition: all 0.2s;" onmouseover="this.style.opacity='0.8'" onmouseout="this.style.opacity='1'" title="{{ __('Click to view member profile, tasks & work time') }}">
                                    <div style="width: 36px; height: 36px; border-radius: 10px; background: var(--nx-palm-900); display: flex; align-items: center; justify-content: center; font-weight: 700; font-size: 12px; color: white; box-shadow: var(--nx-shadow-sm); flex-shrink: 0;">
                                        {{ strtoupper(substr($m->user->name, 0, 2)) }}
                                    </div>
                                    <div>
                                        <div style="color: var(--nx-palm-900); font-size: 13px; font-weight: 700; display: flex; align-items: center; gap: 4px;">
                                            <span>{{ $m->user->name }}</span>
                                            <span class="material-symbols-rounded" style="font-size: 13px; opacity: 0.6;">visibility</span>
                                        </div>
                                        @if($m->user->nickname)
                                            <div style="font-size: 11px; color: var(--nx-text-muted); font-family: 'IBM Plex Mono', monospace;">{{ $m->user->nickname }}</div>
                                        @endif
                                    </div>
                                </div>
                            </td>
                            <td style="font-size: 12px; color: var(--nx-text-secondary); font-family: 'IBM Plex Mono', monospace;">{{ $m->user->email }}</td>
                            <td>
                                @if($memberDept)
                                    <div style="display: flex; flex-direction: column; gap: 2px;">
                                        ...
                                        @if($memberTeam)
                                            <span style="font-size: 10px; color: var(--nx-text-muted); font-weight: 600; display: inline-flex; align-items: center; gap: 3px; margin-inline-start: 8px;">
                                ...
                                @else
                                    <span style="color: var(--nx-text-muted); font-size: 11px; font-style: italic;">— {{ __('Not Assigned') }} —</span>
                                @endif
                            </td>
                            <td style="font-size: 12px; font-weight: 500; color: var(--nx-text-secondary);">
                                {{ $profile?->job_title ?? '—' }}
                            </td>
                            <td>
                                <span class="nav-badge-pill" style="background: rgba(60, 107, 76, 0.12); color: var(--nx-palm-500); font-weight: 600;">{{ $m->role->name ?? __('Company Admin') }}</span>
                            </td>
                            <td>
                                @if($m->status === 'active')
