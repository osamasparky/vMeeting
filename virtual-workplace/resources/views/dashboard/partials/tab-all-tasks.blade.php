                        ...
                        <tr class="alltask-row" 
                            ...
                            <td style="padding: 14px 16px;">
                                <div style="display: flex; gap: 6px; align-items: center;" onclick="event.stopPropagation();">
                                    <x-btn variant="primary" size="sm" onclick="startTaskTimer('{{ $t->project_id }}', '{{ $t->id }}', '{{ addslashes($t->title) }}', '{{ addslashes($t->project->name ?? 'Project') }}')" icon="play_arrow">
                                        {{ __('Timer') }}
                                    </x-btn>
                                    <x-btn variant="secondary" size="sm" onclick="openTaskDetails('{{ $t->id }}')" icon="visibility">
                                        {{ __('Inspect') }}
                                    </x-btn>
                                    <x-btn variant="secondary" size="sm" :iconOnly="true" icon="more_horiz" onclick="openTaskContextMenu(event, '{{ $t->id }}', '{{ $t->project_id }}', '{{ addslashes($t->title) }}')" title="{{ __('More Actions') }}" />
                                </div>
                            </td>
                        </tr>
                    @empty
            ...
            <div class="kanban-column" 
                ...
                <div class="kanban-cards-container" id="global-kanban-col-{{ $statusKey }}" style="display: flex; flex-direction: column; gap: 10px; flex: 1; min-height: 120px;">
                    ...
                    @forelse($colTasks as $t)
                        <x-task-card :task="$t" context="alltasks" :can-edit="true" :is-manager="$user->isSuperAdmin() || $membership->role?->slug === 'company_admin'" />
                    @empty
